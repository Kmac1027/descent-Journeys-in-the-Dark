...
function Canvas({ chosenQuest, collisionDetection }) {
  ...
  useEffect(() => {
    ...
    function drawLevel() {
      ...
      ctx.drawImage(startAreaLabelId, labels.start_area.x, labels.start_area.y, labels.start_area.width, labels.start_area.height);
      if (labels.area1) {
        ctx.drawImage(area1LabelId, labels.area1.x, labels.area1.y, labels.area1.width, labels.area1.height);
      }
      if (labels.area2) {
        ctx.drawImage(area2LabelId, labels.area2.x, labels.area2.y, labels.area2.width, labels.area2.height);
      }
      if (labels.area3) {
        ctx.drawImage(area3LabelId, labels.area3.x, labels.area3.y, labels.area3.width, labels.area3.height);
      }
      if (labels.area4) {
        ctx.drawImage(area4LabelId, labels.area4.x, labels.area4.y, labels.area4.width, labels.area4.height);
      }
      if (labels.area5) {
        ctx.drawImage(area5LabelId, labels.area5.x, labels.area5.y, labels.area5.width, labels.area5.height);
      }
      if (labels.area6) {
        ctx.drawImage(area6LabelId, labels.area6.x, labels.area6.y, labels.area6.width, labels.area6.height);
      }
      if (labels.area7) {
        ctx.drawImage(area7LabelId, labels.area7.x, labels.area7.y, labels.area7.width, labels.area7.height);
      }
      if (labels.area8) {
        ctx.drawImage(area8LabelId, labels.area8.x, labels.area8.y, labels.area8.width, labels.area8.height);
      }
    ...
    function drawUnexploredAreas() {
      ...
      for (let area in unexplored) {
        for (let placement in unexplored[area])
          ctx.drawImage(blackoutId, unexplored[area][placement].x, unexplored[area][placement].y, unexplored[area][placement].width, unexplored[area][placement].height)
      }
    }
